...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('You are not logged in! Plesae log in to get access.', 401)
    );
  }
  ...
  // 4) if user change password after the token was issue
  if (currentUser.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppError('Password changed Session TimeOut! Please login again', 401)
    );
  }
